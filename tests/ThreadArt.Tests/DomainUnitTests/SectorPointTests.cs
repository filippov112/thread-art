...
public class SectorPointTests
{
    ...
    {
        Assert.Equal(expected, new SectorPoint(new(ax, ay), width, height) == new SectorPoint(new(bx, by), width, height));
        Assert.Equal(!expected, new SectorPoint(new(ax, ay), width, height) != new SectorPoint(new(bx, by), width, height));
        Assert.Equal(expected, new SectorPoint(new(ax, ay), width, height).Equals(new SectorPoint(new(bx, by), width, height)));
        Assert.Equal(expected, new SectorPoint(new(ax, ay), width, height).GetHashCode() == new SectorPoint(new(bx, by), width, height).GetHashCode());
        Assert.Equal(expected, new SectorPoint(new(ax, ay), width, height).ToString() == new SectorPoint(new(bx, by), width, height).ToString());
    }
    ...
    {
        Assert.Equal(expected, new SectorPoint(new(x, y), width, height).Sector);
    }
}
